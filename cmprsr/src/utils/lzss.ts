...
function isMatchFoundInWindows(window: string, match: string) {
	return window.includes(match)
}

function getMaximalMatchLengthAndOffset(
	window: string,
	lookAheadBuffer: string,
	min_match_length: number = MIN_MATCH_LENGTH,
	max_match_length: number = MAX_MATCH_LENGTH,
	...
	while (tentativeMatchLength <= max_match_length && !maximalMatchFound) {
		if (!isMatchFoundInWindows(window, currentLookAheadBufferSubString)) {
	...
	export function compress(
		...
		while (lookAheadBuffer.length >= MIN_MATCH_LENGTH) {
			...
			if (isMatchFoundInWindows(window, lookAheadBuffer)) {
			...
			// If no match is found in the window, append to output the UNCOMPRESSED flag and the character in current position
				...
				outputAddition =
					UNCOMPRESSED_FLAG +
					getNumberBinaryForm(currentCharacter.charCodeAt(0))
	...
	if (import.meta.vitest) {
		...
		describe('lzss', () => {
			it('compresses and decompresses', () => {
				...
				expect(decompress(compress('AABCBBABC').compressed)).toEqual(
					'AABCBBABC'
				)
				...
				expect(decompress(compress('Hello Hel').compressed)).toEqual(
					'Hello Hel'
				)
