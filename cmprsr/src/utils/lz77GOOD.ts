...
export function encode(inputStream: string): LZ77Encoded {
		...
		for (
			...
			if (matchIndex === -1 || matchLength === lookAheadBuffer.length) {
				...
				window = inputStream.slice(
					Math.max(0, codingPosition - DEFAULT_WINDOW_SIZE),
					codingPosition
				)
				break
...
export function decode(encodedText: LZ77Encoded): string {
	...
	for (const [pointer, character] of encodedText) {
		decodedText += character ?? encodedText[pointer[0] + pointer[1]][1]
	}
	return decodedText
}
